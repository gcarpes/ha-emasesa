...
class EmasesaDataUpdateCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self) -> dict:
        try:
            ...
            raise UpdateFailed(f"Error de autenticación: {err}") from err
    ...
    async def _inject_statistics(self, dias: list) -> None:
        ...
        for dia in dias:
            ...
            for lectura in dia.get("detalle", []):
                hora = int(lectura.get("hora", 0))
                consumo_m3 = round((lectura.get("consumo", 0) or 0) / 1000, 6)
                try:
                    dt_local = datetime.strptime(f"{fecha_str} {hora:02d}:00:00", "%Y-%m-%d %H:%M:%S").replace(tzinfo=TZ_MADRID)
        ...
        for dt, consumo_m3 in registros:
            ...
            statistics.append(StatisticData(start=dt, state=consumo_m3, sum=running_sum, mean=None, min=None, max=None))
        metadata = StatisticMetaData(
            has_mean=False, has_sum=True,
            name=f"EMASESA Agua {self._contrato}",
            source="emasesa", statistic_id=statistic_id,
            unit_of_measurement=UnitOfVolume.CUBIC_METERS,
        )
        _LOGGER.info("Inyectando %d registros horarios en %s", len(statistics), statistic_id)
